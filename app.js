// โหลด Library Google Charts สำหรับ Gantt
...
function fixTimezoneDate(dateStr) {
    if (!dateStr) return '';
    if (dateStr.length === 10) return dateStr; // ถ้าเป็น YYYY-MM-DD อยู่แล้วให้ผ่านเลย
...
function loginGuest() { appState.user = { id: 'guest', name: 'ผู้เข้าชมทั่วไป', role: 'Guest', profile_img: '' }; initApp(); }
...
async function initApp() {
    ...
    if (data.status === 'success') {
        // ประยุกต์ฟังก์ชันแก้ Timezone ทันทีที่โหลดข้อมูลเสร็จ
...
function renderGantt() {
    ...
    google.charts.setOnLoadCallback(() => {
        ...
        data.addColumn('string', 'Resource'); // เอาชื่อประเภทมาใส่ตรงนี้
        ...
        tasks.forEach(t => {
            ...
            // ป้องกัน Date Object คลาดเคลื่อน
...
function renderCalendar() {
    ...
    appState.calendarInstance = new FullCalendar.Calendar(document.getElementById('calendar'), {
        ...
        events: appState.activities.map(a => {
            ...
            // เพิ่ม +1 วันเพื่อให้ระบายสีคลุมเต็มวันสุดท้าย (ข้อจำกัดของ FullCalendar)
            if (endDateStr) {
...
function renderUserTable() {
    let html = `<button onclick="openModal('user')" class="mb-4 bg-gray-800 text-white px-4 py-2 rounded-lg"><i class="fas fa-user-plus"></i> เพิ่ม User</button><table class="w-full text-sm text-left"><tr class="bg-gray-100"><th class="p-3 border-b">ชื่อ</th><th class="p-3 border-b">Username</th><th class="p-3 border-b">สิทธิ์</th><th class="p-3 border-b">จัดการ</th></tr>`;
    appState.users.forEach(u => html += `<tr class="border-b"><td class="p-3">${u.name}</td><td class="p-3">${u.username}</td><td class="p-3">${u.role}</td><td class="p-3"><button onclick="openModal('user','${u.id}')" class="text-blue-500 mr-3"><i class="fas fa-edit"></i></button><button onclick="deleteRecord('deleteUser','${u.id}')" class="text-red-500"><i class="fas fa-trash"></i></button></td></tr>`);
    document.getElementById('userTableContainer').innerHTML = html + `</table>`;
}

function renderTypeTable() {
    let html = `<button onclick="openModal('type')" class="mb-4 bg-green-600 text-white px-4 py-2 rounded-lg"><i class="fas fa-tags"></i> เพิ่มประเภท</button><table class="w-full text-sm text-left"><tr class="bg-gray-100"><th class="p-3 border-b w-24 text-center">สี</th><th class="p-3 border-b">ชื่อประเภท</th><th class="p-3 border-b">จัดการ</th></tr>`;
    appState.types.forEach(t => html += `<tr class="border-b"><td class="p-3 text-center"><span class="w-6 h-6 inline-block rounded" style="background:${t.color}"></span></td><td class="p-3">${t.name}</td><td class="p-3"><button onclick="openModal('type','${t.id}')" class="text-blue-500 mr-3"><i class="fas fa-edit"></i></button><button onclick="deleteRecord('deleteType','${t.id}')" class="text-red-500"><i class="fas fa-trash"></i></button></td></tr>`);
...
function openModal(type, id = null) {
    ...
    if(type === 'activity') {
        ...
        ['id','title','type_id','assignee','progress','status','result'].forEach(k => document.getElementById(`act-${k.split('_')[0]}`).value = item[k] || (k==='progress'?0:''));
...
document.getElementById('activityForm').addEventListener('submit', async(e) => { e.preventDefault(); if(appState.user.role !== 'Guest') saveData('saveActivity', { id: document.getElementById('act-id').value, title: document.getElementById('act-title').value, type_id: document.getElementById('act-type').value, assignee: document.getElementById('act-assignee').value, start_date: document.getElementById('act-start').value, end_date: document.getElementById('act-end').value, progress: document.getElementById('act-progress').value, status: document.getElementById('act-status').value, result: document.getElementById('act-result').value }, 'activity'); });
document.getElementById('userForm').addEventListener('submit', async(e) => { e.preventDefault(); const p = { id: document.getElementById('u-id').value, name: document.getElementById('u-name').value, username: document.getElementById('u-user').value, role: document.getElementById('u-role').value, status: document.getElementById('u-status').value }; const pass = document.getElementById('u-pass').value; if(pass) p.password = await hashPassword(pass); if(!p.id && !pass) return Swal.fire('ข้อมูลไม่ครบ', 'กรุณาตั้งรหัสผ่านสำหรับ User ใหม่', 'error'); saveData('saveUser', p, 'user'); });
document.getElementById('typeForm').addEventListener('submit', async(e) => { e.preventDefault(); saveData('saveType', { id: document.getElementById('t-id').value, name: document.getElementById('t-name').value, color: document.getElementById('t-color').value }, 'type'); });
document.getElementById('profileForm').addEventListener('submit', async (e) => { e.preventDefault(); if(appState.user.role === 'Guest') return; const pass = document.getElementById('profPass').value; const img = document.getElementById('profImg').value; let p = { id: appState.user.id }; if (img) p.profile_img = img; if (pass) p.new_password = await hashPassword(pass); Swal.fire({ title: 'บันทึก...', didOpen: () => Swal.showLoading() }); const res = await callAPI('updateProfile', p); if(res.status === 'success') { Swal.fire('สำเร็จ', '', 'success'); if(img) { appState.user.profile_img = img; document.getElementById('userAvatar').src = img; } document.getElementById('profPass').value = ''; } });
...
async function deleteRecord(action, id) {
    if(await Swal.fire({ title: 'ลบข้อมูล?', icon: 'warning', showCancelButton: true }).then(r => r.isConfirmed)) {
        Swal.fire({ title: 'กำลังลบ...', didOpen: () => Swal.showLoading() });
        if((await callAPI(action, {id})).status === 'success') {
            const data = await callAPI('getData');
            appState.activities = (data.activities||[]).map(a=>{a.start_date=fixTimezoneDate(a.start_date); a.end_date=fixTimezoneDate(a.end_date); return a;}); appState.users = data.users||[]; appState.types = data.types||[];
